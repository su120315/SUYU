...
(function initGlobalSearch() {
  ...
  const searchData = [
    { group: '页面导航', icon: 'user', title: '关于我', desc: '个人介绍、技能、项目、爱好', action: () => scrollToSection('#about') },
    { group: '页面导航', icon: 'zap', title: '技能', desc: '前端、JavaScript、UI设计等技能展示', action: () => scrollToSection('#skills') },
    { group: '页面导航', icon: 'briefcase', title: '作品', desc: '项目作品集', action: () => scrollToSection('#projects') },
    { group: '页面导航', icon: 'heart', title: '爱好', desc: '兴趣爱好展示', action: () => scrollToSection('#hobbies') },
    { group: '页面导航', icon: 'image', title: '相册', desc: '照片画廊浏览', action: () => scrollToSection('#gallery') },
    { group: '页面导航', icon: 'message-circle', title: '留言板', desc: '互动交流留言', action: () => scrollToSection('#comments') },
    { group: '页面导航', icon: 'mail', title: '联系方式', desc: '社交账号与联系方式', action: () => scrollToSection('#contact') },
    { group: '功能模块', icon: 'book-open', title: '漫画阅读器', desc: '多源聚合、离线下载、追漫书架', action: () => goto('comic.html') },
    { group: '功能模块', icon: 'book-marked', title: '小说阅读', desc: '搜索海量小说、书架、书签', action: () => goto('novel.html') },
    { group: '功能模块', icon: 'wrench', title: '工具箱', desc: '计算器、计时器、二维码等工具', action: () => goto('tools.html') },
    { group: '功能模块', icon: 'code', title: '编程主页', desc: '编程项目作品展示', action: () => { window.open('https://su120315.github.io/SUYU3/', '_blank'); } },
    { group: '快捷搜索', icon: 'search', title: '搜索漫画...', desc: '在漫画库中搜索漫画', action: () => goto('comic.html') },
    { group: '快捷搜索', icon: 'search', title: '搜索小说...', desc: '在小说库中搜索小说', action: () => goto('novel.html') },
    { group: '外部链接', icon: 'globe', title: '百度', desc: '百度一下，你就知道', action: () => { window.open('https://www.baidu.com', '_blank'); } },
    { group: '外部链接', icon: 'play-circle', title: '哔哩哔哩', desc: 'B站视频分享', action: () => { window.open('https://www.bilibili.com', '_blank'); } },
    { group: '外部链接', icon: 'book-open', title: '知乎', desc: '有问题，上知乎', action: () => { window.open('https://www.zhihu.com', '_blank'); } },
  ];
  ...
  function renderResults(query) {
    ...
    for (let i = 0; i < searchData.length; i++) {
      ...
      if (!query) {
        ...
        const match = item.title.toLowerCase().includes(query) || item.desc.toLowerCase().includes(query);
        if (match) {
          filteredItems.push(item);
          if (!groups[item.group]) groups[item.group] = [];
          groups[item.group].push(item);
    ...
    if (filteredItems.length === 0) {
      results.innerHTML = '<div class="global-search-empty">没有找到相关结果</div>';
      activeIndex = -1;
      return;
    }
    ...
    for (const groupName in groups) {
      html += '<div class="global-search-group-label">' + groupName + '</div>';
      for (let i = 0; i < groups[groupName].length; i++) {
        const item = groups[groupName][i];
        const isActive = flatIdx === 0;
        html += '<div class="global-search-item' + (isActive ? ' active' : '') + '" data-index="' + flatIdx + '">'
          + '<div class="global-search-item-icon"><i data-lucide="' + item.icon + '" style="width:18px;height:18px;"></i></div>'
          + '<div class="global-search-item-text">'
          + '<div class="global-search-item-title">' + highlight(item.title, query) + '</div>'
          + '<div class="global-search-item-desc">' + highlight(item.desc, query) + '</div>'
          + '</div></div>';
        flatIdx++;
      }
    }
  ...
  input.addEventListener('keydown', function(e) {
    if (e.key === 'ArrowDown') {
      e.preventDefault();
      if (activeIndex < filteredItems.length - 1) activeIndex++;
      updateActive();
  ...
  modal.addEventListener('click', function(e) {
    if (e.target === modal || e.target.classList.contains('global-search-backdrop')) {
      closeSearch();
    }
  });
